...
from asn4 import As # my current AS Class
...
class show_as_info(framework.Framework):
    ...
    def show_entry_fields(self):
        ...
        # open file 
        ip = open(filename, 'a')
        print (thisas.lat,thisas.lon)
        ip.write(str(thisas.lat)+", "+str(thisas.lon)+'], 6);\n')
        ip.close()
        ...
        string2 = "], { \n        color: 'red',\n        fillColor: '#f03',\n        fillOpacity: 0.5,\n        radius: 50000\n        }).addTo(map);\n"
        ip.write(string1+str(thisas.lat)+', '+str(thisas.lon)+string2)
        ip.close()
        # show all ip locations on map
        result = {}
        ipinfo = {}
        #print (self.owner)
        ipinfo = thisas.get_prefixes()
        ...
        inetnums = thisas.get_inetnums()
        print(inetnums)
        inums_length = len(inetnums)
        inets = {}
        if inums_length > 0:
            for ipaddr in inetnums:
                   #lat = inetnums[key]
                #lon = inetnums[key.value]  
