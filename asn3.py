...
class As:
    ...
    # TODO: Class d is No longer in use
    class d():
        # used by test
    ...
    def inetnum_exists(self, inum):
        ...
        cursor = mydb.cursor()
        cursor.execute("SELECT id FROM inetnums WHERE subnet = %s",(inum,))
    ...
    def get_info_from_ipinfo(self):
        ...
        for i in range(num_inums +1):
            if i != 0:
                print ("INETNUM is ###", inetnum[i])
                inetnum_exist = self.inetnum_exists(inetnum[i])
    ...
    def add_inetnums_to_db(self,inum,cidr,org_id, lat,lon):
        ...
        cursor = mydb.cursor()
        sql = "INSERT INTO inetnums(subnet,owner,region, lat, lon, prefix) VALUES (%s, %s, %s, %s, %s,%s)"
        val = (inum, org_id , self.region, lat, lon, cidr )
        cursor.execute(sql,val)
