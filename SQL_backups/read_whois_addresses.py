...
def sql_address_write(address,org):
    #print (record_id)
    ...
    cursor = mydb.cursor()
    sql = "INSERT INTO addresses(organisation, address) VALUES ( %s, %s);"
    val = (org, address )
    #print (org,address)
    #cursor.execute(sql,val)
    #mydb.commit()
...
if __name__ == "__main__":
    ...
    for id in range(2,126618):
        ...
        #print ("ORG IS ",org)
        #org = "ORG-BPIS1-RIPE"
        cmd = "whois -Br --sources RIPE "+org + " > test"
        address = []
        info = os.system(cmd)
        ...
        for add in range(1,i):
            address = (info.split("address:")[1].split("\n")[0].strip())  
            print ( "Address is ",add-1,address.strip())
            print ("org is ",org)
